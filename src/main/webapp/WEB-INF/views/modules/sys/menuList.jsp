			...
			<tbody><c:forEach items="${list}" var="menu">
				<tr id="${menu.id}" pId="${menu.parent.id ne '1'?menu.parent.id:'0'}">
					...
					<shiro:hasPermission name="sys:menu:edit"><td nowrap>
						<a style="color: #0b2c89" href="${ctx}/sys/menu/form?id=${menu.id}">修改</a>
						<a style="color: #0b2c89" href="${ctx}/sys/menu/delete?id=${menu.id}" onclick="return confirmx('要删除该菜单及所有子菜单项吗？', this.href)">删除</a>
						<a style="color: #0b2c89" href="${ctx}/sys/menu/form?parent.id=${menu.id}">添加下级菜单</a>
