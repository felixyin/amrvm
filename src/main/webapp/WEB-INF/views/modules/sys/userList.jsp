		...
		<c:forEach items="${page.list}" var="user">
			<tr>
				...
				<shiro:hasPermission name="sys:user:edit"><td>
    				<a style="color: #0b2c89" href="${ctx}/sys/user/form?id=${user.id}">修改</a>
					<a style="color: #0b2c89" href="${ctx}/sys/user/delete?id=${user.id}" onclick="return confirmx('确认要删除该用户吗？', this.href)">删除</a>
					<a style="color: #0b2c89" onclick="showItemsDialog('${ctx}/iot/iotWatermeterUser/form?id=${user.id}','职责分配')">
						职责分配
					</a>
