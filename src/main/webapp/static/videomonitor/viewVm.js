...
var iiTypes = iotVideoMonitorConfig.windowCount;       //默认窗口数
$(function () {
    // 检查插件是否已经安装过
    var iRet = WebVideoCtrl.I_CheckPluginInstall();
    if (-2 == iRet) {
        alert("您的Chrome浏览器版本过高，不支持NPAPI插件，请用chrome54以下版本和IE10以上浏览器访问！");
        return;
    } else if (-1 == iRet) {
    ...
    // 初始化插件参数及插入插件
    WebVideoCtrl.I_InitPlugin(document.body.clientWidth - 210, document.body.clientHeight, {
...
$(document.body).keydown(function (evt) {
    var kc = evt.keyCode;
    if (kc === 39) {
        var channel = parseInt($('#channels').val());
        channel++;
        var size = $('#channels').children().size();
        if (channel > size) {
            channel = 0;
        }
        $('#channels').children().filter(function (i, n) {
        ...
        WebVideoCtrl.I_FullScreen(fullScreen);
...
function clickLogin() {
    var itl = setInterval(function () {
        if (WebVideoCtrl && WebVideoCtrl.I_Login) {
            clearInterval(itl);
            var szIP = iotVideoMonitorConfig.ip,
                szPort = iotVideoMonitorConfig.httpPort,
                szUsername = iotVideoMonitorConfig.account,
                szPassword = iotVideoMonitorConfig.password;
            ...
            var iRet = WebVideoCtrl.I_Login(szIP, 1, szPort, szUsername, szPassword, {
                success: function (xmlDoc) {
                    ...
                    setTimeout(function () {
                        ...
                        getChannelInfo();
                        // 改变窗口数量
                        $('#wndNum').children().filter(function () {
                            $(this).val() == iotVideoMonitorConfig.windowCount
                        }).prop('selected', true);

                        iiTypes = iotVideoMonitorConfig.windowCount;
                        changeWndNum(iotVideoMonitorConfig.windowCount);
                        // 自动显示画面
                        PlayAll();
                    }, 10);
